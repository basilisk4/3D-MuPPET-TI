...
def DLCInference(InferFrame,inBox,dlc_liveObj,samPredictor,CropSize):
    """Inference for DLC"""
    ...
    return DLCPredict2DList,outBox

def RunDLCLoop(dlc_liveObj,OutbboxList,OutConfList,SequenceNum,DatasetPath,CropSize,startFrame,TotalFrames,VisualizeIndex,ScaleBBox,samPredictor):
    ...
    for i in tqdm(range(len(OutbboxList))):
        ...
        for x in range(len(SequenceObj.camObjects)):
            ...
            for box in boxesList:
                ...
                DLCPredict2D,outBox= DLCInference(Img,box,dlc_liveObj,samPredictor,CropSize)
                Key2DPredList.append(DLCPredict2D)    
                outBoxList.append(outBox)  

            Key2DPred = np.array(Key2DPredList)
            boxesList=outBoxList   
        ...
        cv2.imwrite(img=frame, filename="vis/"+str(SequenceNum)+"-"+str(i)+".jpg")
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
...
def RunInference3DPOP(YOLOPath,DLCPath,SamType,SamCkpt,SequenceNum,DatasetPath,CropSize,startFrame=0,TotalFrames = 1800,VisualizeIndex = 0,ScaleBBox=1):
    ...
    Points3DDict,Tracker3DOutDict,Detection2DOutDict, Tracking2DOutDict,Points3DList,Points2DDict = RunDLCLoop(dlc_liveObj,OutbboxList,OutConfList,SequenceNum,DatasetPath,CropSize,startFrame,TotalFrames,VisualizeIndex,ScaleBBox,predictor)
...
if __name__ == "__main__":
    ...
    Points3DDict,Tracker3DOutDict,Detection2DOutDict, Tracking2DOutDict,Points3DList,Points2DDict=RunInference3DPOP(YOLOPath,DLCPath,SamType,SequenceNum,DatasetPath,CropSize,SamCkpt=SamCkpt,startFrame=0,TotalFrames = 250,VisualizeIndex = 0,ScaleBBox=1)
