...
def ResizeAndPad(image, size):
    ...
    bbox = [left, top, left + new_w, top + new_h]
    return padded_image, bbox


def segmentSam(image, processor, model):
    resized_image, prompt_box = ResizeAndPad(image, 1024)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    inputs = processor(resized_image, input_boxes=[[prompt_box]], return_tensors="pt")
    #inputs = processor(resized_image, input_boxes=[[prompt_box]],point_coords=[[512,512]], point_lables=[[1]], return_tensors="pt")
    inputs = {k: v.to(device) for k, v in inputs.items()}
    with torch.no_grad():
        outputs = model(**inputs, multimask_output=False)

    pred_mask = outputs.pred_masks.squeeze(1)  # (1, H, W)
    pred_mask=nn.interpolate(pred_mask,resized_image.size,mode="bilinear",align_corners=False)
    pred_mask = pred_mask[..., prompt_box[1]: prompt_box[3], prompt_box[0]: prompt_box[2]]
    pred_mask=nn.interpolate(pred_mask,(image.size[1],image.size[0]),mode="bilinear",align_corners=False)
    pred_mask = torch.sigmoid(pred_mask).squeeze().cpu().numpy()
    binary_mask = (pred_mask > 0.5).astype(np.uint8) * 255
    output_img = Image.fromarray(binary_mask).convert("1")

    
    #pred_mask = torch.sigmoid(pred_mask).squeeze().cpu().numpy()
    #binary_mask = (pred_mask > 0.5).astype(np.uint8) * 255  # Convert to [0, 255] image
    #output_img = Image.fromarray(binary_mask).convert("1")
    #output_img=output_img.resize(resized_image.size, resample=Image.Resampling.BILINEAR)#resize to original size
    #output_img=output_img.crop(prompt_box) # remove padding
    #output_img=output_img.resize(image.size, resample=Image.Resampling.BILINEAR)#resize to original size
    return output_img
...
def DLCInference(InferFrame,box,dlc_liveObj,CropSize,SamProc,SamMod):
    """Inference for DLC"""
    ...
    Mask = segmentSam(Crop,SamProc,SamMod)
    Crop.save('1.jpg')
    Mask.save('0.jpg')   
    Mask=np.array(Mask)
...
def RunDLCLoop(dlc_liveObj,OutbboxList,OutConfList,SequenceNum,DatasetPath,CropSize,startFrame,TotalFrames,VisualizeIndex,ScaleBBox,SamProc,SamMod):
    ...
    for i in tqdm(range(len(OutbboxList))):
        ...
        for x in range(len(SequenceObj.camObjects)):
            ...
            for box in boxesList:
                ...
                DLCPredict2D= DLCInference(Img,box,dlc_liveObj,CropSize,SamProc,SamMod)
...
def RunInference3DPOP(YOLOPath,DLCPath,SamType,SequenceNum,DatasetPath,CropSize,startFrame=0,TotalFrames = 1800,VisualizeIndex = 0,ScaleBBox=1, SamCkpt=""):
    ...
    torch.cuda.empty_cache()
    dlc_proc = Processor()
    dlc_liveObj = DLCLive(DLCPath, processor=dlc_proc)
    
    SamProc = SamProcessor.from_pretrained("facebook/sam-vit-"+SamType)
    device=device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    SamMod = SamModel.from_pretrained("facebook/sam-vit-"+SamType)
    if not SamCkpt=="":
        SamMod.load_state_dict(torch.load(SamCkpt, map_location=device))
    SamMod.to(device)
    SamMod.eval()
    
    Points3DDict,Tracker3DOutDict,Detection2DOutDict, Tracking2DOutDict,Points3DList,Points2DDict = RunDLCLoop(dlc_liveObj,OutbboxList,OutConfList,SequenceNum,DatasetPath,CropSize,startFrame,TotalFrames,VisualizeIndex,ScaleBBox,SamProc,SamMod)
...
if __name__ == "__main__":
    ...
    DLCPath = args.DLCweight
    SamType = args.SamType
    SamCkpt= args.SamCkpt

    CropSize = (320,320)


    Points3DDict,Tracker3DOutDict,Detection2DOutDict, Tracking2DOutDict,Points3DList,Points2DDict=RunInference3DPOP(YOLOPath,DLCPath,SamType,SequenceNum,DatasetPath,CropSize,SamCkpt=SamCkpt,startFrame=0,TotalFrames = 10,VisualizeIndex = 0,ScaleBBox=1)
